...
function getYesterdayBRTString() {
    const now = new Date();
...
async function getSupabaseStats(dateString) {
    try {
        console.log(`🔍 [Stats] Buscando produtos enviados em: ${dateString}`);

        const { data, error } = await supabase
            .from('produtos')
            .select('loja, bazar, favorito, novidade, hora_entrada')
            .ilike('hora_entrada', `%${dateString}%`);

        if (error) throw error;

        console.log(`✅ Itens brutos encontrados: ${data.length}`);
        if (data.length > 0) {
            console.log(`Primeiro item: ${JSON.stringify(data[0])}`);
        }
...
async function diagnose() {
    const yesterday = getYesterdayBRTString();
    const stats = await getSupabaseStats(yesterday);
    ...
    console.log(`\n--- DIAGNÓSTICO DE QUOTA PARA ${yesterday} ---`);
